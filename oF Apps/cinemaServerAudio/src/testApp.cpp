...
static bool pastScreen(ofPtr<ofxBox2dBaseShape> shape) {
    CustomData *custom = (CustomData *)shape->getData();
...
void testApp::setup() {
    ...
    mainMenu->addImageButton("Noi-Sea Bird", "noi-sea-bird.png", false, 200, 16);
...
void testApp::guiEvent(ofxUIEventArgs &e)
{
    ...
    if (kind == OFX_UI_WIDGET_IMAGEBUTTON) {
        ...
        if (name == "Noi-Sea Bird" && button->getValue()) {
...
//
        if (msg.getAddress() == "/join") {
            ...
            if (std::find(knownClients.begin(), knownClients.end(), incomingPlayer) == knownClients.end()) {
                ...
                msgToPlayer.addIntArg(GAME_CONTROL_AUDIO);
        ...
        if (msg.getAddress() == "/sound") {
            if (msg.getNumArgs() == 2) {
                int playerNumber = msg.getArgAsInt32(0);
                if (players.size() > playerNumber) {
                    float power = msg.getArgAsFloat(1);
                    float modPower = ofMap(power, 0, 1, -10, 10);
                    players[playerNumber].get()->addForce(ofVec2f(0,-modPower), 1);
                }
            }
        }
    ...
    switch (gameState) {
        case WAITING_FOR_PLAYERS:
            break;
        case PLAYING_GAME_1:
           // ofRemove(invaders, pastScreen);
            if (invaders.size() < AMOUNT_OF_INVADERS) {
                newInvader();
                cout << "creating new invader" << endl;
            }
            playersLeft = 0;
    ...
    if(e.a != NULL && e.b != NULL) {
        ...
        if (data1 == NULL && data2 != NULL) {
            ...
            //colliding against each other
                ...
                if (data1->type == FOOD_TYPE && data2->type == PLAYER_TYPE) {
                    //food is data1, player is data2
                    data1->remove = true;
                    players[data2->id].get()->score+= food[data1->id].get()->getWidth();
                    // newFood();
                }
                else if (data1->type == PLAYER_TYPE && data2->type == FOOD_TYPE){
                    //food is data2, player is data1
                    data2->remove = true;
                    players[data1->id].get()->score+= food[data2->id].get()->getWidth();
                    // newFood();
                }
                else if (data1->type == INVADER_TYPE && data2->type == PLAYER_TYPE) {
                    cout << "player touched invader" << endl;
                    players[data2->id].get()->hit = true;

                }
                else if (data2->type == INVADER_TYPE && data1->type == PLAYER_TYPE) {
                    cout << "invader touched player" << endl;
                    players[data1->id].get()->hit = true;
                }
            }
        }
    }
...
void testApp::newInvader() {
    ofPtr<Invader> inv = ofPtr<Invader>(new Invader);
    inv.get()->setPhysics(10, 1, 0);
    float iSize = ofRandom(50,100);
    float h = ofRandom(ofGetHeight()/2, ofGetHeight());
    float w = ofRandom(ofGetScreenWidth());
    inv.get()->setup(box2d.getWorld(), w, h, iSize, iSize);
    cout << "Created Invader at " << h << endl;
    inv.get()->setVelocity(ofRandom(-10, 10), ofRandom(5,5));
    inv.get()->setData(new CustomData());
    inv.get()->setupCustom(invaders.size());
